...
def merkez(cnts, frame):
    for c in cnts:
        ...
        if M["m00"] != 0:
            ...
            cY = int(M["m01"] / M["m00"])
            if area > 20000:
                cv2.putText(frame, "engel", (cX, cY), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
...
def hareket(mesafe,aci):
    ...
    ser.close()
    time.sleep(5)
def hareketli_cisim(frame1,frame2):

    diff = cv2.absdiff(frame1, frame2)
    cnts,_=cisim_detect(diff)
    for c in cnts:
        (x, y, w, h) = cv2.boundingRect(c)
        if cv2.contourArea(c) < 700:
            continue
        cv2.rectangle(frame1, (x,y), (x+w,y+h), (0,255,0), 2)
        cv2.putText(frame1, "engel", (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
        print("hareketli cisim var")
...
def main():
    ...
    while True:
        ...
        #time.sleep(1)
        cnts,blur=cisim_detect(frame)

        hareketli_cisim(frame,frame2)
        merkez(cnts, frame)
        #time.sleep(1)
        if len(centers) > 5:
            del centers[:]
        ...
        if len(robot_center) == 1:
            if len(cop) > 0:
                ...
                if len(engel) > 0:
                    ...
                    #roi = frame[cop[cop_sirasi[0]][1]:robot_center[0][1], robot_center[0][0]:cop[cop_sirasi[0]][0]]
                    #cv2.line(frame, (cop[len(cop) - 1][0], cop[len(cop) - 1][1]),
                     #        (cop[len(cop) - 1][0], robot_center[0][1]), (255, 0, 0), 2)
                    #cv2.line(frame, (cop[len(cop) - 1][0], robot_center[0][1]), (robot_center[0][0], robot_center[0][1]),
                     #        (255, 0, 0), 2)
                    ...
                    if len(led_merkez) > 0:
                        ...
                        d = oklid_uzaklik(robot_center[0][0], robot_center[0][1], cop[cop_sirasi[0]][0],
                                          cop[cop_sirasi[0]][1])
